...
### device：指明要挂载的设备；
1. 设备文件：例如/dev/sda5
1. 卷标： -L 'LABEL', 例如 -L 'MYDATA'
1. UUID, -U 'UUID'：例如 -U '0c50523c-43f1-45e7-85c0-a126711d406e'
1. 伪文件系统名称： proc, sysfs, devtmpfs, configfs
### dir：挂载点
- 事先存在；建议使用空目录
- 进程正在使用中的设备无法被卸载
### 常用选项
...
- -B, --bind 绑定目录到另一个目录上
### -o options
挂载文件系统的选项，多个选项使用逗号分隔
- async 异步模式 
- sync 同步模式，内存更改时，同时写磁盘
- atime/noatime 时间戳，包含目录和文件
